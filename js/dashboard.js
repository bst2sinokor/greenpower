...
function renderKPI() {
    ...
    setKPI('kpi-profit', fmt(profit / 10000, 0), '만원', profit >= 0);
    setKPI('kpi-margin', fmt(margin, 1), '%', margin >= 0);
    setKPI('kpi-trucks', fmt(avgTrucks, 1), '대');
    setKPI('kpi-landfill', fmt(totalLandfill, 0), 'm³');
...
function renderIsometric() {
    ...
    const phases = phaseConfigs.map(cfg => ({
        num: cfg.phase_num,
        label: `${cfg.phase_num}공구`,
        used: latest ? (latest[`phase${cfg.phase_num}_used_m3`] || 0) : 0,
...
function renderPhaseCards(phases) {
    ...
    phases.forEach(p => {
        ...
        <div class="phase-info-card" id="phase-card-${p.num}">
            ...
            <div class="phase-stat">
                <span>허가량</span>
                <span style="font-weight:600;">${fmt(p.total, 1)} <small style="font-weight:400;color:var(--text-muted)">m³</small></span>
            </div>
            <div class="phase-stat">
                <span>기 매립량</span>
                <span style="font-weight:700;color:${pctColor}">${fmt(p.used, 1)} <small style="font-weight:400">m³</small>
                    <span style="font-size:10px;color:${pctColor};">(${pct1}%)</span>
                </span>
            </div>
            <div class="phase-stat">
                <span>잔여량</span>
                <span style="font-weight:700;color:var(--accent-dark)">${fmt(remaining, 1)} <small style="font-weight:400">m³</small>
                    <span style="font-size:10px;color:var(--accent-dark);">(${remPct1}%)</span>
                </span>
            </div>
            ${p.permitExpiry ? `<div class="phase-stat" style="margin-top:4px;padding-top:4px;border-top:1px solid var(--border);">
                <span>허가 만료</span><span style="font-size:11px;">${p.permitExpiry}</span>
            </div>` : ''}
    ...
    const totalPctColor = totalRatio < 0.5 ? '#16a34a' : totalRatio < 0.75 ? '#d97706' : totalRatio < 0.9 ? '#ea580c' : '#dc2626';
    ...
    <div class="phase-info-card" style="background:linear-gradient(135deg,#1a3a5c,#1e4d7b);color:white;border-color:#1a3a5c;">
        ...
        <div class="phase-stat" style="color:rgba(255,255,255,0.8);">
            <span>총 허가량</span>
            <span style="font-weight:700;color:white;">${fmt(totalPermit, 1)} <small style="font-weight:400;opacity:0.7">m³</small></span>
        </div>
        <div class="phase-stat" style="color:rgba(255,255,255,0.8);">
            <span>총 기 매립량</span>
            <span style="font-weight:700;color:#fde047;">${fmt(totalUsed, 1)} <small style="font-weight:400">m³</small>
                <span style="font-size:10px;">(${totalPct1}%)</span>
            </span>
        </div>
        <div class="phase-stat" style="color:rgba(255,255,255,0.8);">
            <span>총 잔여량</span>
            <span style="font-weight:700;color:#4ade80;">${fmt(totalRemain, 1)} <small style="font-weight:400">m³</small>
...
총 매립량: ${fmt(sum('landfill_volume_m3'), 1)} m³
침출수 발생 합계: ${fmt(sum('leachate_generated_m3'), 1)} m³
